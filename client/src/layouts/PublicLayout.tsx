      ...
      <header className="sticky top-0 z-50 bg-white border-b border-slate-200 shadow-sm">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          <Link to={`/${orgSlug}`} className="flex items-center gap-3">
            ...
            {tenant?.name && (
              <span className="font-semibold text-slate-800 text-sm hidden sm:block">{tenant.name}</span>
            )}
          </Link>
          <nav className="flex items-center gap-4 text-sm text-slate-600">
            <Link to={`/${orgSlug}`} className="hover:text-blue-700 transition-colors">Events</Link>
          </nav>
        </div>
      </header>
