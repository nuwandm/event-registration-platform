...
export function PlatformLoginPage() {
  ...
  const { mutate, isPending, error } = useMutation({
    mutationFn: (data: FormValues) => authApi.platformLogin(data),
    onSuccess: (res) => {
      const { token, admin, orgSlug } = res.data.data!;
      if (admin.role === 'super_admin') {
        setAuth(token, admin, undefined);
        navigate('/superadmin');
      } else {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8">
          ...
          <p className="text-slate-500 text-sm mb-6">
            Enter your credentials — you'll be taken directly to your portal.
          </p>
        ...
        <div className="mt-4 text-center">
          <Link to="/" className="inline-flex items-center gap-1.5 text-xs text-slate-400 hover:text-slate-200 transition-colors">
