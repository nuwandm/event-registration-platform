...
import { CalendarDays, MapPin, Users, CreditCard, Building2, ArrowRight, Ticket } from 'lucide-react';
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-10 pb-28">
      ...
      <div className="fixed bottom-6 right-4 z-50 flex flex-col items-end gap-3">
        {/* Track Registration button — always visible */}
        <button
          onClick={() => document.querySelector<HTMLInputElement>('input[placeholder*="EVT"]')?.scrollIntoView({ behavior: 'smooth', block: 'center' }) || document.querySelector<HTMLInputElement>('input[placeholder*="EVT"]')?.focus()}
        ...
        >
          ...
          <span className="hidden sm:flex flex-col items-start leading-tight">
            <span className="text-[10px] font-normal opacity-80">Already registered?</span>
            <span>Track your Registration</span>
          </span>
        </button>

        {/* WhatsApp button — only when number is set */}
        {event.whatsappNumber && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="flex items-center gap-2 bg-[#25D366] hover:bg-[#1ebe5d] text-white px-3 py-3 sm:px-4 rounded-full shadow-lg hover:shadow-xl transition-all duration-200 font-semibold text-sm"
